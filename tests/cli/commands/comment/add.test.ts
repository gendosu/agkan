...
describe('setupCommentAddCommand', () => {
  ...
  it('should add a comment successfully', async () => {
    ...
    try {
      await program.parseAsync(['node', 'test', 'task', 'comment', 'add', String(task.id), 'Progress is good']);
    } finally {
    ...
    expect(comments[0].content).toBe('Progress is good');
  });
  ...
  it('should output JSON when --json flag is used', async () => {
    ...
    try {
      await program.parseAsync(['node', 'test', 'task', 'comment', 'add', String(task.id), 'JSON note', '--json']);
    } finally {
      console.log = originalLog;
      process.exit = originalExit;
    }
    ...
    expect(parsed.data.content).toBe('JSON note');
  });
  ...
  it('should fail when task not found', async () => {
    ...
    try {
      await program.parseAsync(['node', 'test', 'task', 'comment', 'add', '99999', 'Note']);
    } finally {
      console.log = originalLog;
      process.exit = originalExit;
    }
