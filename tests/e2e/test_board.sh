...
start_board_server() {
    ...
    start_background_process node "$SCRIPT_DIR/bin/agkan" board --port "$BOARD_PORT"
    BOARD_PID=$BACKGROUND_PROCESS_PID
    ...
    while ! curl -s "http://localhost:$BOARD_PORT/" > /dev/null 2>&1; do
        ...
        if [ $retries -ge 50 ]; then
            ...
            stop_background_process "$BOARD_PID"
            kill_port "$BOARD_PORT"
            BOARD_PID=""
            return 1
        fi
...
test_board_title_option() {
    ...
    start_background_process node "$SCRIPT_DIR/bin/agkan" board --port "$title_port" --title "My Project"
    title_pid=$BACKGROUND_PROCESS_PID
    ...
    while ! curl -s "http://localhost:$title_port/" > /dev/null 2>&1; do
        ...
        if [ $retries -ge 50 ]; then
            ...
            stop_background_process "$title_pid"
            kill_port "$title_port"
            return 1
        fi
        sleep 0.2
