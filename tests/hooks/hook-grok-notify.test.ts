...
const SCRIPT = resolve(__dirname, '../../src/hooks/hook-grok-notify.mjs');
...
const grokStopPayload = JSON.stringify({
  hook_event_name: 'Stop',
  reason: 'end_turn',
  stopHookActive: false,
});

describe('hook-grok-notify.mjs', () => {
  ...
  it('posts complete and replies with {} when no target status is configured and reason is end_turn', async () => {
    const before = svr.captured.length;
    const { code, stdout } = await runHook(grokStopPayload, baseEnv);
    expect(code).toBe(0);
    expect(stdout).toBe('{}');
  ...
  it('does NOT post when reason is channel_closed (session shutdown)', async () => {
    const before = svr.captured.length;
    const payload = JSON.stringify({ hook_event_name: 'Stop', reason: 'channel_closed' });
  ...
  it('does NOT post when reason is shutdown', async () => {
    const before = svr.captured.length;
    const payload = JSON.stringify({ hook_event_name: 'Stop', reason: 'shutdown' });
    const { code, stdout } = await runHook(payload, baseEnv);
    expect(code).toBe(0);
    expect(stdout).toBe('{}');
    expect(svr.captured.length).toBe(before);
  ...
  it('does NOT post when the stdin payload is not JSON', async () => {
    const before = svr.captured.length;
  ...
  it('does NOT post when BOARD_TASK_ID is not numeric', async () => {
    const before = svr.captured.length;
    const { stdout } = await runHook(grokStopPayload, { ...baseEnv, BOARD_TASK_ID: 'abc' });
    expect(stdout).toBe('{}');
    expect(svr.captured.length).toBe(before);
  });
  ...
  it('does NOT post when board env vars are missing', async () => {
    const before = svr.captured.length;
    const { stdout } = await runHook(grokStopPayload, {
      BOARD_TASK_ID: '5',
      BOARD_API_URL: `http://127.0.0.1:${svr.port}`,
    });
    expect(stdout).toBe('{}');
  ...
  it('does NOT post when a target status is configured but not yet reached', async () => {
    ...
    const { stdout } = await runHook(grokStopPayload, { ...baseEnv, BOARD_TARGET_STATUS: 'review' });
    expect(stdout).toBe('{}');
    expect(svr.captured.length).toBe(before);
  });
  ...
  it('posts complete when the target status has been reached and reason is end_turn', async () => {
    const before = svr.captured.length;
  ...
  it('posts complete when the task advanced past the target to a terminal status and reason is end_turn', async () => {
    const before = svr.captured.length;
    svr.setStatus('done');
    const { stdout } = await runHook(grokStopPayload, { ...baseEnv, BOARD_TARGET_STATUS: 'review' });
    expect(stdout).toBe('{}');
