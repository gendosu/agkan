...
Each agent CLI must be installed and authenticated separately. Setting `agent` to any other value raises an error: `Invalid agent "<value>". Must be one of: claude, codex, agy, grok`.
...
> **grok side effect**: like agy, grok loads hooks globally from `~/.grok/hooks/*.json`. Running a board session with `agent: grok` writes `~/.grok/hooks/agkan-board-stop.json` containing a Stop hook (existing files like `herdr.json` are preserved untouched). This is only written while the board itself has hooks enabled; a manually-run `grok` outside the board is unaffected because the hook checks board-specific environment variables before doing anything.
...
|-------|------|-------------|
| `cli` | string | `claude`, `codex`, `agy`, or `grok`. The cli that runs a task which selects this model |
...
- a row's `cli` is not `claude`, `codex`, `agy`, or `grok`
...
| Value | grok CLI flag |
|-------|---------------|
| (not set) / `auto` | `--permission-mode auto` |
...
Unlike agy, grok natively supports `auto`, so the unset default uses `--permission-mode auto`.
