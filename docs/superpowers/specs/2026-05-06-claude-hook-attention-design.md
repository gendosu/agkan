...
- Claude CLI のhooks機構は `Stop` の `stop_reason` で `end_turn` / `tool_use` / `max_tokens` を区別するが、**ユーザーへの質問と通常完了は両方 `end_turn`** となるため stop_reason のみでは区別不可。
...
| 完了判定 | transcript の最後の `tool_use` 検査 | `AskUserQuestion` か否かで質問中 / 完了を確定判定 |
...
```
1. stdin から hook input JSON を読む（transcript_path, stop_reason, session_id を取得）
2. stop_reason !== "end_turn" なら何もせず exit 0
