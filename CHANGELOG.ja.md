...
### 変更
- `agkan init` が生成する run のモデルを `gpt-5.6-sol` から `gpt-6-astra` に更新し、README と設定例も生成内容に揃えた (#761)
...
- `agent` に `claude` / `codex` に加えて `agy` を追加。組み込みの `modelCatalog` に `agy models` の14モデルを追加した。各行は effort がモデルID自体に埋め込まれているため既定では effort のオーバーライドを受け付けない（プロジェクト側で `.agkan.yml` に effort を持つ独自の agy 行を定義することは可能で、その場合は agy 汎用の `--effort` フラグがそのまま渡される）。`agent: agy` で起動したボードセッションは、agy 唯一のグローバル設定ファイル `~/.gemini/config/hooks.json` に Stop フックを登録する（claude の `--settings` や codex の `--config notify=` のような起動時フラグが agy には無いため、ユーザーが既に持つ他のフックとマージして書き込む）。これによりセッションは自力で終了できるようになり、初期プロンプトは `agy -i <prompt>` で渡されるため、セッションは以降の入力を受け付ける対話状態のまま維持される。agy には claude の `auto` パーミッションモードに相当するものがないため、`permissionMode` が未設定または `auto` の場合は agy に `--dangerously-skip-permissions` を渡す。agy の対話的な request-review 動作を維持したい場合は `permissionMode: default` を設定する
