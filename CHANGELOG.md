...
### Added
- Add grok row `grok-4.7` (efforts `low`, `medium`, `high`, `xhigh`) to the built-in `modelCatalog`, ahead of `grok-4.6`, and use it in the grok example of the `agkan init` template. The existing grok rows are unchanged (#759)
...
- Add `agy` as a third `agent` option alongside `claude` and `codex`, with its 14 models (from `agy models`) appended to the built-in `modelCatalog`. Each agy row's effort is already baked into the model id, so none of the built-in rows accept a separate effort override (a project can still define its own agy rows with efforts in `.agkan.yml`; agy's generic `--effort` flag is passed through when one is selected). Board sessions launched with `agent: agy` register a Stop hook in agy's single global `~/.gemini/config/hooks.json` (merging with any hooks the user already has there, since agy has no per-invocation flag like claude's `--settings` or codex's `--config notify=`) so the session terminates on its own, and the initial prompt is passed via `agy -i <prompt>`, which keeps the session interactive for further input. Because agy has no equivalent of claude's `auto` permission mode, `permissionMode` unset or `auto` is passed to agy as `--dangerously-skip-permissions`; set `permissionMode: default` to keep agy's interactive request-review behavior
